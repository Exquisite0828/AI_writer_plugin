...
DEMO_DIR = REPO_ROOT / "examples" / "technical_solution_demo_fixture"
...
VERIFY_REPORT_READABLE_KEYS = {"details", "message", "warnings", "blocking_failures"}
...
def readable_verify_report_strings(value: object, key: str = "") -> list[str]:
    if isinstance(value, dict):
        strings: list[str] = []
        for child_key, child_value in value.items():
            strings.extend(readable_verify_report_strings(child_value, child_key))
        return strings
    if isinstance(value, list):
        strings = []
        for item in value:
            strings.extend(readable_verify_report_strings(item, key))
        return strings
    if isinstance(value, str) and key in VERIFY_REPORT_READABLE_KEYS:
        return [value]
    return []


@pytest.fixture(scope="module")
def technical_solution_run(tmp_path_factory: pytest.TempPathFactory) -> Path:
    runs_dir = tmp_path_factory.mktemp("technical-solution-runs")
...
def test_technical_solution_fixture_uses_current_task_schema() -> None:
    expected_inputs = {
...
def test_technical_solution_full_run_artifact_set_and_task_type(technical_solution_run: Path) -> None:
    rules = get_document_type_rules("technical_solution")
    manifest = read_json(technical_solution_run / "manifest.json")
    task_brief = read_json(technical_solution_run / "task_brief.json")

    for relative_path in REQUIRED_ARTIFACTS:
        assert (technical_solution_run / relative_path).exists(), relative_path
...
def test_technical_solution_user_outputs_do_not_leak_hara_terms(technical_solution_run: Path) -> None:
    for relative_path in USER_FACING_OUTPUTS:
        text = (technical_solution_run / relative_path).read_text(encoding="utf-8")
        for forbidden in FORBIDDEN_HARA_LEAKAGE:
            assert forbidden not in text, f"{forbidden!r} leaked into {relative_path}"
    verify_report = read_json(technical_solution_run / "verify" / "verify_report.json")
    for text in readable_verify_report_strings(verify_report):
        for forbidden in FORBIDDEN_HARA_LEAKAGE:
            assert forbidden not in text, f"{forbidden!r} leaked into verify_report.json readable field: {text}"


def test_technical_solution_planning_outputs_do_not_leak_hara_terms(technical_solution_run: Path) -> None:
...
def test_technical_solution_sample_is_not_used_as_fact_source(technical_solution_run: Path) -> None:
    inventory = read_json(technical_solution_run / "inputs" / "input_inventory.json")
    source_index = read_json(technical_solution_run / "knowledge" / "source_index.json")
    evidence_map = read_json(technical_solution_run / "plans" / "evidence_map.json")
    citation_plan = read_json(technical_solution_run / "plans" / "citation_plan.json")
    final_report = (technical_solution_run / "final" / "final_report.md").read_text(encoding="utf-8")
    draft = (technical_solution_run / "draft" / "full_draft.md").read_text(encoding="utf-8")
...
def test_technical_solution_reference_is_not_project_fact_source(technical_solution_run: Path) -> None:
    inventory = read_json(technical_solution_run / "inputs" / "input_inventory.json")
    source_index = read_json(technical_solution_run / "knowledge" / "source_index.json")
    citation_plan = read_json(technical_solution_run / "plans" / "citation_plan.json")
...
def test_technical_solution_critical_claims_require_confirmation(technical_solution_run: Path) -> None:
    rules = get_document_type_rules("technical_solution")
    final_review = (technical_solution_run / "review" / "final_review.md").read_text(encoding="utf-8")
    verify_report = read_json(technical_solution_run / "verify" / "verify_report.json")
    final_report = (technical_solution_run / "final" / "final_report.md").read_text(encoding="utf-8")
    delivery_summary = (technical_solution_run / "final" / "delivery_summary.md").read_text(encoding="utf-8")
    combined = f"{final_review}\n{json.dumps(verify_report, ensure_ascii=False)}\n{final_report}\n{delivery_summary}"
    deliverable_text = f"{final_review}\n{final_report}\n{delivery_summary}"
...
def test_technical_solution_final_status_and_candidate_updates_stay_conservative(technical_solution_run: Path) -> None:
    rules = get_document_type_rules("technical_solution")
    verify_report = read_json(technical_solution_run / "verify" / "verify_report.json")
    revision_plan = read_json(technical_solution_run / "revision_plan.json")
    final_report = (technical_solution_run / "final" / "final_report.md").read_text(encoding="utf-8")
    delivery_summary = (technical_solution_run / "final" / "delivery_summary.md").read_text(encoding="utf-8")
    candidate_profile = (technical_solution_run / "learning" / "candidate_profile_update.yaml").read_text(encoding="utf-8")
    promotion_report = (technical_solution_run / "learning" / "promotion_report.md").read_text(encoding="utf-8")
    ...
    assert final_report.startswith("# 技术方案文档最终交付包")
    assert f"Status: {rules.default_final_status}" in final_report
    assert "## 核心证据边界" in final_report
    assert rules.default_final_status in delivery_summary
