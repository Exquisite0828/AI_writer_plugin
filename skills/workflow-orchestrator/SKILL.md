...
- **薄编排器长期上下文**：主 Agent 只保留稳定编排规则、ProgressLedger 短账本、StepContextPackage 路径/hash、StepResult / ReviewResult 短摘要和人工闸门状态；不得粘贴 artifact 正文，不读取 artifact 正文，不得批量读取 step canonical，不得把动态 artifact 内容、review 明细或输入材料全文带回长期上下文。
...
- **StepWorkerDispatch / ingest worker pilot**：仅 `ingest` stage 的 `step-input-materials`、`step-material-inventory`、`step-source-index` 使用 StepWorkerDispatch：`runs/<run_id>/orchestration/worker_dispatches/<stage>/<step>.json`。主 Agent 只把 dispatch/context package 路径交给 worker；非 `ingest` stage 暂不宣称已完成 worker pilot。
