...
- **薄编排器长期上下文**：主 Agent 只保留稳定编排规则、路径/hash、StepResult / ReviewResult 短摘要和人工闸门状态；不得粘贴 artifact 正文，不读取 artifact 正文，不得批量读取 step canonical，不得把动态 artifact 内容、review 明细或输入材料全文带回长期上下文。
