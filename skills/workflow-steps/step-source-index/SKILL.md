...
- `inputs/input_inventory.json` 是本步读取输入材料的唯一来源；只处理其中 `parse_status=parsed` 的记录。
- 读取原文时必须使用 `files[].path` 中已经解析好的路径；该路径必须可直接读取，且不得再按当前 cwd、`runs/<run_id>/` 或 subagent cwd 重新拼接。
- 禁止从 `run_dir`、`manifest.json`、文件名模式或 `inputdoc/datas` 目录名反推输入根目录。
- 若 `files[].path` 不存在、不是可读文件或仍是依赖 cwd 的相对路径，必须把对应材料写入 `knowledge_gaps.md` 并报告 P1 路径锚定缺陷；不得 Glob 全仓库、不得在 `runs/<run_id>/inputdoc/` 下猜测输入位置。
...
本步执行结束前，必须由 Claude Code **新开一个独立 subagent**，审核本步已产出的 artifacts。subagent 默认只审核，不重新生成目录、索引或 gap；只有发现 P0/P1 时才允许进入局部修订。

派发 subagent 时，主 agent 必须在任务说明中显式传入以下绝对路径，并要求 subagent 原样使用：

1. `project_root` — 当前项目/任务目录的绝对路径（例如包含 `task.yaml` 与 `inputdoc/` 的目录）。
2. `run_dir` — 当前 `runs/<run_id>/` 的绝对路径。
3. `input_inventory_path` — 当前 `runs/<run_id>/inputs/input_inventory.json` 的绝对路径。
4. `step_output_dir` — 当前 Step 3 输出目录（通常为 `<run_dir>/knowledge`）。

subagent 默认只能读取这些审核对象：`input_inventory_path`、`<step_output_dir>/source_index.json`、`<step_output_dir>/provenance_index.json`、`<step_output_dir>/document_tocs/`、`<step_output_dir>/knowledge_gaps.md`。需要抽查原文时，只能通过 `input_inventory_path` 读取 `files[].path`，并只使用这些路径访问原文。subagent 不得从 `run_dir` 拼接 `inputdoc/datas`，不得把当前工作目录当作输入根目录，路径不存在时必须记录 issue/state 后停止相关修订，而不是反复搜索或猜测。

审核通过且无 P0/P1 时，subagent 只能写入 `runs/<run_id>/subagent/step-source-index/state.json`，不得重写 `source_index.json`、`provenance_index.json`、`document_tocs/` 或 `knowledge_gaps.md`。P2/P3 只记录为待用户确认的问题，不得自动修订本步 artifacts。

### A. 自主任务分解与进度跟踪（将 human 移出 loop）

subagent 必须先对「审核任务」做动态任务分解，并在 `state.json` 中以 `review_state` 跟踪进度。`revision_state` 只有在审核发现 P0/P1 时才执行；无 P0/P1 时记录 `revision_required=false`。P2/P3 不触发 A2 修订。
...
2. **分解与执行（按 issue / artifact）**：每个修订子任务必须绑定 `issue_id`、`target_artifact`、`changed_paths`。只读取修订所需的最小 artifact / 原文片段；不得重新遍历全部文档、不得重新生成全部 `document_tocs`、不得整步重写 `source_index.json`。
...
1. 派发新 subagent（fresh context），交付本步 artifact（见上「产出 artifacts」）、上述绝对路径上下文与本文「边界与约束」+ 所加载子 skill 的「B 审核检查项」作为审核标准。
